...
const ProjectHome = () => {
  ...
  const subMenuItems = [
    {
      key: "/project/tools/plugins",
      icon: <LogoutOutlined />,
      text: "Plugins",
      path: `/project/tools/plugins`,
    },
    {
      key: "/project/tools/actions",
      icon: <LogoutOutlined />,
      text: "Actions",
      path: `/project/tools/actions`,
    },
  ];
  ...
  const filteredKeys = [
    location.pathname === `/project` || location.pathname === `/project/`
      ? `/project/home`
      : location.pathname,
    location.pathname.includes("tools") && `/project/tools`,
    !location.pathname.includes("tools/actions") &&
      location.pathname.includes("tools") &&
      `/project/tools/plugins`,
  ];

  const [selectedKey, setSelectedKey] = useState(
    filteredKeys.filter((item) => Boolean(item)) as string[],
  );
  const [isOpen, setOpen] = useState(
    selectedKey
      .filter((item) => typeof item === "string")
      .some((item) => (item as string).includes("tools")),
  );
  ...
  useEffect(() => {
    ...
    if (
      location.pathname === `/project/playground` &&
      assistantId &&
  ...
  return (
    <Layout style={{ minHeight: "100vh", maxHeight: "100vh" }}>
      <Header className="header">
        <div
          className={`left-header1 ${collapsed ? "collapsed-header" : ""}`}
        ...
        >
          ...
          {!collapsed ? (
            <div className="arrowiconsParent" onClick={toggleCollapsed}>
              <ArrowIcons />
            </div>
          ) : showTaskingAi ? (
            <TaskingAi onClick={toggleCollapsed} />
          ) : (
            <div className="arrowiconsParent" onClick={toggleCollapsed}>
              {" "}
              <RightArrow />{" "}
        ...
        <div className="right-header">
          ...
          <div className="button-group-href">
            ...
            <Button
              ...
              className="document-button cancel-button"
              onClick={() =>
                window.open("https://docs.tasking.ai/api/", "_blank")
              }
            >
              API Reference
            </Button>
      ...
      <Layout>
          ...
          <div className="menu-page">
            ...
            >
              ...
              >
                <Menu.Item
                  key={`/project/home`}
                  icon={<HomeIcon className="svg-icons" />}
                >
                  <Link to={`/project/home`}>Home</Link>
                </Menu.Item>
                <Menu.Item
                  key={"/project/playground"}
                  icon={<Playground className="svg-icons" />}
                >
                ...
                <Menu.Item
                  key={"/project/models"}
                  icon={<IconData className="svg-icons" />}
                >
                  <Link to={"/project/models"}>Models</Link>
                </Menu.Item>
                <Menu.Item
                  key={"/project/collections"}
                  icon={<Retrieval className="svg-icons" />}
                >
                  <Link to={"/project/collections"}>Retrieval</Link>
                </Menu.Item>
                <Menu.Item
                  key={"/project/tools"}
                  icon={<Plugin className="svg-icons" />}
                ...
                ></Menu.Item>
                <Menu.Item
                  key={`/project/assistants`}
                  icon={<Assistant className="svg-icons" />}
                >
                ...
                <Menu.Item
                  key={"/project/apikeys"}
                  className="api-key-menu"
                  icon={<Apikeys className="svg-icons" />}
                >
                  <Link to={"/project/apikeys"}>API Keys</Link>
                </Menu.Item>

                <Menu.Item
                  icon={<Back className="svg-icons" />}
                  onClick={handleBack}
                  className="orgination"
                  key="/"
                >
                ...
                <Menu.Item
                  icon={<TaskingCloud className="svg-icons-taskingai" />}
                  className="taskingCloud"
                  key="/taskingCloud"
                >
                  <a href="http://www.tasking.ai" target="blank">
                    Try TaskingAI Cloud
                  </a>
                </Menu.Item>
                <Menu.Item
        ...
        >
          <span>
            Are you sure you want to log out of your TaskingAI account?
          </span>
        </Modal>
      </Layout>
    </Layout>
