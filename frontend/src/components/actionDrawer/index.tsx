...
const ActionDrawer = forwardRef((props: any, ref: any) => {
  ...
  const [resetButtonShow, setResetButtonShow] = useState(
    actionId ? true : false,
  );
  ...
  return (
    <div className={styles["action-drawer"]}>
      <div className={styles["top"]}>
        ...
        {!actionId ? (
          <div className={styles["label-description"]}>
            ...
            >
              {" "}
              {t("projectActionSchemaDescLink")}
            </a>
            .{t("projectActionSchemaDesc")}
            <a
              href="https://docs.tasking.ai/docs/guide/tool/action"
              target="_blank"
              rel="noopener noreferrer"
              className={"href"}
            >
              {" "}
              {t("projectActionSchemaDescLinkEnd")}{" "}
            </a>
            {t("projectActionToLearnMore")}
          </div>
        ) : (
          <div className={styles["label-description"]}>
            {t("projectActionEditDesc")}
          </div>
        )}
        ...
        ></TextArea>
        <div
          className={`${styles["desc-action-error"]} ${showTipError ? styles.show : ""}`}
        >
          Schema is required
        </div>
      </div>
      <div className={styles["bottom"]}>
        ...
        <div className={styles["label-description"]}>
          {t("projectActionAuthenticationType")}
        </div>
        {resetButtonShow && (
          <div
        ...
        <div className={resetButtonShow ? styles.resetBackground : undefined}>
          ...
          {radioValue !== "none" && (
            ...
            >
              {radioValue !== "custom" ? (
                <span className={styles["desc-description"]}>
                  Authorization{" "}
                </span>
              ) : (
                <Input
              ...
              <Input
                prefix={
                  <span style={{ color: "#999" }}>
                    {radioValue !== "custom" && titleCase(radioValue)}
                  </span>
                }
                value={Authentication}
                placeholder="<Secret>"
                onChange={hangleChangeAuthorization}
                style={{ width: "83%" }}
              ></Input>
            </div>
          )}
