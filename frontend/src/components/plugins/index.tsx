import { Space, Spin, Tooltip, Button, Form, Input, Drawer } from "antd";
...
function Plugins() {
  ...
  const { tooltipEditTitle, tooltipDeleteTitle, tooltipPluginTitle } =
    tooltipTitle();
  ...
  columns.push({
    ...
    render: (_: any, record: any) => (
      <Space size="middle">
        ...
        <div
          onClick={
            JSON.stringify(record.display_credentials) !== "{}"
              ? () => handleEdit(record)
              : undefined
          }
          className={`table-edit-icon ${JSON.stringify(record.display_credentials) === "{}" && styles.disabledButton}`}
        >
          {JSON.stringify(record.display_credentials) !== "{}" ? (
            <Tooltip
              placement="bottom"
  ...
  const handleEdit = async (record: any) => {
    ...
    const data: any = bundilesList.find(
      (item: any) => item.bundle_id === record.bundle_id,
    ) as any;
  ...
  const handleClickPlugin = (pluginId: string, pluginName: string) => {
    ...
    setPluginDesc(
      (pluginListData as any[]).find((item: any) => item.plugin_id === pluginId)
        .description,
    );
    const inputSchematemp = (pluginListData as any[]).find(
          ...
          <div className={styles.inputWithLabelParent}>
            <div className={styles.inputWithLabel}>
              ...
              <div className={styles.inputWithLabelInner}>
                <div className={styles.frameWrapper}>
                  <div className={styles.frameContainer}>
                    <div className={styles.logoParent}>
                      <img
                        loading="lazy"
                        src={(cachedImages as any)[bundleId]}
                        alt=""
                ...
                <div className={styles.desc}>{bundleDesc}</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div className={styles.content1}>
          <div className={styles.left}>
            {pluginListData.map((item: any, index) => (
              <div
                key={index}
                onClick={() => {
                  handleClickPlugin(item.plugin_id, item.name);
                }}
                className={`${styles.pluginName} ${pluginId === item.plugin_id && styles.pluginId}`}
              >
                {item.name}
              </div>
            ))}
          ...
          <div className={styles.right}>
            <div className={styles.topContent}>
              ...
              {!isShowBundle && (
                <div className={styles.pluginId}>
                  {bundleId}/{pluginId}{" "}
                  <CopyOutlined
                    className="icon-copy"
                    onClick={() => handleCopy(`${bundleId}/${pluginId}`)}
                  />
                </div>
              )}
            </div>
      ...
      <div className={styles.editForm}>
        ...
        {JSON.stringify(credentialsSchema) !== "{}" && (
          <div>
            ...
            <div
              className={styles["label-desc"]}
              style={{ marginBottom: "24px" }}
            >
              All plugin credentials are encrypted at rest with AES-256 and in
            ...
            >
              {credentialsSchema &&
                Object.entries(credentialsSchema).map(
                  ([key, property]: [any, any]) => (
                    <Form.Item
                      label={key}
                      key={key}
                      name={key}
  ...
  return (
    <div className={styles["actions"]}>
      ...
      <Drawer
        ...
        open={openEditDrawer}
        title={bundleName + " / " + t("projectPluginsTitle")}
        className={styles.openLookDrawer}
      >
        <ComponentsData />
      </Drawer>
      <Drawer
        title={t("projectPluginEditPlugin")}
        footer={[
          <Button
            key="cancel"
          ...
          <Button
            ...
            onClick={handleEditFormConfirm}
            className={`next-button ${styles.button}`}
          >
            {t("confirm")}
          </Button>,
        ]}
        open={openEditFormDrawer}
        closeIcon={
          <img
            src={closeIcon}
            alt="closeIcon"
        ...
        className={styles.openEditDrawer}
      >
        <EditForm />
      </Drawer>
      <DeleteModal
        open={OpenDeleteModal}
        describe={`${t("projectPluginDeleteDesc")} ${deleteValue}? ${t("projectDeleteProjectDesc")}`}
        title={t("projectPluginDeleteTitle")}
        projectName={deleteValue}
