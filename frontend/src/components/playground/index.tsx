...
import {
  setPlaygroundSelect,
  setPlaygroundAssistantId,
} from "@/Redux/actions/playground.ts";
...
function Playground() {
  ...
  const { assistantPlaygroundId } = useSelector(
    (state: any) => state.assistantId,
  );
  ...
  const combineObjects = (item: any, arr: any) => {
    ...
    if (item.object === "MessageGenerationLog") {
      ...
      const currentIsDuplicate = previousItems.some(
        (existingItem: any) => existingItem.event_id === newItem.event_id,
      );
  ...
  const fetchAssistantsList = async (value?: any) => {
    ...
    if (value) {
      ...
      res = (await getAssistantsList({
        limit: assistantLimit || 20,
      })) as FullApiResponse;
    }
  ...
  const initialFunction = async () => {
    ...
    if (assistantId && assistantId === assistantPlaygroundId) {
      ...
      setAssistantName(
        localStorage.getItem("assistantName") || "Untitled Assistant",
      );
      try {
      ...
      try {
        ...
        if (res2.data.length > 0) {
          const res: any = await getHistoryMessage(
            assistantId,
            res2.data[0]?.chat_id,
            param,
          );
  ...
  const fetchHistoryMessage = async (
    ...
    try {
      ...
      localStorage.setItem(
        "contentTalk",
        JSON.stringify([...data, ...JSON.parse(contentTalk1)]) as any,
      );
  ...
  const handleEditAssistant = async () => {
    ...
    setSelectedPluginGroup(
      tools
        ?.filter((item: any) => item.type === "plugin")
        .map((item: any) => item.id?.split("/")[1]),
    );
    ...
    setSelectedActionsRows(
      tools.map((item: any) => {
        return { type: item.type, value: item.id, name: item.name };
      }),
    );
  ...
  const handleRequest = async () => {
    ...
    const params = {
      model_id: Array.isArray(originalModelData)
        ? originalModelData[0].slice(-8)
        : originalModelData,
      name: drawerName || "",
  ...
  const handleCreateMessage = async (flag?: any) => {
    ...
    const lastData = Array.isArray(
      contentTalk[contentTalk.length - 1]?.content.text,
    );
    let lastMessage: boolean = false;
    if (lastData) {
      const lastMessage1 =
        contentTalk[contentTalk.length - 1].content.text[
          contentTalk[contentTalk.length - 1].content.text.length - 1
        ];
  ...
  const handleAssistantModalClose = () => {
    ...
    const assistantData = optionList.find(
      (item: any) => item.assistant_id === data,
    );
    if (assistantData) {
  ...
  const handleLodaMore = async () => {
    ...
    localStorage.setItem(
      "listChats",
      JSON.stringify([...listChats, ...res.data]),
    );
  ...
  const handleClickDebug = (item: any) => {
    const inputResult: any = debugArray1.length
      ? debugArray1
      : JSON.parse(localStorage.getItem("inputResult") as any);
    const outputResult: any = debugArray2.length
      ? debugArray2
      : JSON.parse(localStorage.getItem("outputResult") as any);
    if (item.event === "Error Occurred") {
      ...
      const data1 = inputResult?.find(
        (item1: any) => item1.event_id === item.event_id,
      );
      const data2: any = outputResult?.find(
        (item1: any) => item1.event_id === item.event_id,
      );
      if (data1) {
        delete data1.color;
        ...
        const label =
          data1.event_step?.charAt(0).toUpperCase() +
          data1.event_step?.slice(1);
        const label1 = data1.event
          .split("_")
          .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
          .join(" ");
        ...
        const label =
          data2.event_step?.charAt(0).toUpperCase() +
          data2.event_step?.slice(1);
        const label1 = data2.event
          .split("_")
          .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
          .join(" ");
        setCollapseLabel1(`${label1}: ${label}`);
  ...
  return (
    <>
              ...
              <div className={styles["left-content"]}>
                ...
                <div className={styles["bottom"]}>
                  ...
                  <Space.Compact>
                    ...
                    <Input
                      style={{ width: "73%" }}
                      onPressEnter={handleSearchChatId}
                      value={searchChatID}
                      onChange={(e) => handleChangeSearchChatID(e)}
                      placeholder="Enter chat_id"
                      suffix={
                        <SearchOutlined
                          onClick={handleSearchChatId}
                  ...
                  <div className={styles["chats"]}>
                    <div className={styles["chat-message"]}>
                      {listChats?.map((item, index) => (
                        <div
                          key={index}
                          className={`${styles.functionaliconsParent} ${chatId === item.chat_id && styles.chatId}`}
              ...
              <div className={styles["right-content"]}>
                ...
                {chatId && (
                  <Spin spinning={contentTalkLoading}>
                    <div
                      className={`${styles["content-talk"]} playground-content-markdown`}
                      ref={contentRef}
                    >
                      <div
                      ...
                      {contentTalk.map((item, index) => (
                        ...
                        >
                          <div
                            className={`${styles.subText1} ${item.role === "user" ? styles.user : ""}`}
                          >
                          ...
                          {typeof item.content.text === "string" && (
                            <div
                              className={`${styles.text1} ${item.role === "user" ? styles.userInfo : ""}`}
                              style={{ whiteSpace: "pre-line" }}
                            >
                              {checkBoxValue.indexOf(4) !== -1 ? (
                          ...
                          {typeof item.content.text === "object" && (
                            <div
                              className={`text1 ${item.role === "user" ? styles.userInfo : ""}`}
                            >
                              {item.content.text.map(
                                (item1: any, index1: number) => (
                                  <div
                                    key={index1}
                                    className={`${item1.color === "orange" && index === contentTalk.length - 1 ? "orange" : "green"} ${index1 === item.content.text.length - 1 && styles.lastItem}`}
                                  >
                                    {item1.color === "orange" &&
                ...
                <div
                  className={`${styles["content-bottom"]} ${!chatId ? styles.none : ""}`}
                >
                  {imgList.length > 0 && (
                    <div className={styles["upload-list"]}>
                      {imgList.map((item, index) => (
                        <div key={index} className={styles["upload-item"]}>
                          {item.loadingAnim && (
                            <AnimLoadingImg className="anim-loading" />
                          )}
                          <Image
                            style={{
                              filter: item.loadingAnim
                                ? "brightness(50%)"
                                : "brightness(100%)",
                            }}
                            src={item.preview}
                          ...
                          <RemoveIcon
                            ...
                            onClick={() => handleRemoveCloseIcon(item.url)}
                          />
                        </div>
                      ))}
                    </div>
                  ...
                  <div className={styles["button-group"]}>
                    <div style={{ display: "flex" }}>
                      ...
                      <div
                        className={`${styles.formbuttoncancel} ${sendButtonLoading ? styles.loading : ""}`}
                      ...
                      <div
                        className={`${styles.formbuttoncancel} ${styles.button1} ${generateButtonLoading ? styles.loading : ""}`}
      ...
      >
        <DrawerAssistant
          modelName={modelName}
          drawerTitle={t("projectEditAssistant")}
          openDrawer={OpenDrawer}
          selectedActionsSelected={selectedActionsSelected}
          selectedPluginGroup={selectedPluginGroup}
          ...
          selectedRows={originalModelData}
        ...
        footer={[
          <div className="footer-group" key="group">
            ...
            <div>
              ...
              <Button
                key="submit"
                onClick={handleModalCloseConfirm}
                className="next-button"
              >
                {t("confirm")}
              </Button>
      ...
      >
        <ModalTable
          ...
          defaultSelectedRowKeys={
            Array.isArray(selectedModelRows)
              ? selectedModelRows
              : [selectedModelRows]
          }
      ...
      <DeleteModal
        ...
        describe={`${t("deleteItem")} ${t("projectPlaygroundChatLow")} ${chatId}`}
