...
import {
  ...
  setPlaygroundModelName,
  setTemperatureData,
  setMaxTokenData,
  setTopPData,
  setTopKData,
  setStopSequencesData,
} from "@/Redux/actions/playground";
...
function PlaygroundModel() {
  ...
  const {
    ...
    topPValueRedux,
    topKValueRedux,
    stopSequenceValueRedux,
  } = useSelector((state: any) => state.playgroundModelRedux);
    ...
    const fetchData = async () => {
      ...
      if (modelSchemaId && providerId) {
        ...
        if (modelReduxId === modelId && modelId) {
          ...
          const allowedConfigs = JSON.parse(
            localStorage.getItem("allowedConfigs") as any,
          );
          const streaming = JSON.parse(
            localStorage.getItem("streaming") as any,
          );
          const contentModelList = JSON.parse(
            localStorage.getItem("modelContentList") as any,
          );
  ...
  const handleModalConfirm = async (detailData: any) => {
    ...
    navigation(
      `${pathname}?model_id=${detailData.model_id}&model_name=${detailData.name}`,
    );
    setLoading(false);
  };
  const handleGenerate = async () => {
    ...
    if (streamShow && streamSwitch) {
      ...
      let source;
      source = new SSE(
        `${origin}/${project_base_url}/inference/chat_completion`,
        {
          headers: {
      ...
      source.addEventListener("message", (event: any) => {
        if (event.data === "[DONE]") {
          ...
          setContentList((prev) => {
            localStorage.setItem(
              "modelContentList",
              JSON.stringify([...prev, { role: "user", content: "" }]),
            );
  ...
  return (
    <Spin spinning={loading}>
      {selectedModel[0].id ? (
        <div className={styles.playgroundModal}>
            ...
            <div className={styles.leftBottom}>
              <div className={styles.generateOptions}>
                ...
                <div style={{ display: "flex", margin: "12px 0 0 0" }}>
                  ...
                  <div className={styles.responseStream}>{modelSchemaId}</div>
                </div>
                ...
                {streamShow && (
                  <div style={{ marginTop: "12px" }}>
                    <Checkbox
                      value={streamSwitch}
                      defaultChecked={true}
                      onChange={(e) => handleStreamSwitch(e)}
                    />
                    <span style={{ marginLeft: "8px" }}>Stream</span>
                  </div>
                )}
              </div>
              <div style={{ margin: "24px" }}>
                ...
                >
                  {allowedConfigs.includes("temperature") && (
                    ...
                    >
                      <Checkbox
                        value={temperatureCheckbox}
                        defaultChecked={temperatureCheckbox}
                        onChange={handleTemperatureCheckbox}
                      />
                      <div style={{ flex: 1, marginLeft: "12px" }}>
                        <div className={styles.temperature}>
                          <span
                            style={{
                              color: temperatureCheckbox
                                ? "#2B2B2B"
                                : "#BFBFBF",
                            }}
                          >
                          ...
                          {temperatureCheckbox && (
                            <InputNumber
                              ...
                              onChange={(value: number | null) =>
                                setTemperatureValue(value as number)
                              }
                              min={0}
                              max={1}
                        ...
                        {temperatureCheckbox && (
                          <Slider
                            ...
                            step={0.01}
                            value={temperatureValue}
                            onChange={handleTemperatureValue}
                          />
                        )}
                      </div>
                    </div>
                  ...
                  {allowedConfigs.includes("max_tokens") && (
                    ...
                    >
                      <Checkbox
                        value={maxTokenCheckbox}
                        defaultChecked={maxTokenCheckbox}
                        onChange={handleMaxTokenCheckbox}
                      />
                      <div style={{ flex: 1, marginLeft: "12px" }}>
                        <div className={styles.temperature}>
                          <span
                            style={{
                              color: maxTokenCheckbox ? "#2B2B2B" : "#BFBFBF",
                            }}
                          >
                            Max Tokens
                          </span>
                          {maxTokenCheckbox && (
                            <InputNumber
                              ...
                              value={maxTokenValue}
                              onChange={(value: number | null) =>
                                setMaxTokenValue(value as number)
                              }
                              min={1}
                              max={8192}
                            ></InputNumber>
                  ...
                  {allowedConfigs.includes("stop") && (
                    ...
                    >
                      <Checkbox
                        value={stopSequencesCheckbox}
                        defaultChecked={stopSequencesCheckbox}
                        onChange={handleStopSequencesCheckbox}
                      />
                      <div style={{ flex: 1, marginLeft: "12px" }}>
                        <div
                          className={styles.temperature}
                          style={{
                            color: stopSequencesCheckbox
                              ? "#2B2B2B"
                              : "#BFBFBF",
                          }}
                        >
                  ...
                  {allowedConfigs.includes("top_p") && (
                    ...
                    >
                      <Checkbox
                        value={topPCheckbox}
                        defaultChecked={topPCheckbox}
                        onChange={handleTopPCheckbox}
                      />
                      <div style={{ flex: 1, marginLeft: "12px" }}>
                        <div className={styles.temperature}>
                          <span
                            style={{
                              color: topPCheckbox ? "#2B2B2B" : "#BFBFBF",
                            }}
                          >
                            Top P
                          </span>
                  ...
                  {allowedConfigs.includes("top_k") && (
                    ...
                    >
                      <Checkbox
                        value={topKCheckbox}
                        defaultChecked={topKCheckbox}
                        onChange={handleTopKCheckbox}
                      />
                      <div style={{ flex: 1, marginLeft: "12px" }}>
                        <div className={styles.temperature}>
                          <span
                            style={{
                              color: topKCheckbox ? "#2B2B2B" : "#BFBFBF",
                            }}
                          >
                            Top K
                          </span>
            ...
            <div className={styles.content} ref={contentListRef}>
              <div className={styles.contentItem}>
                ...
                <Input.TextArea
                  ...
                  onChange={(e) => handleChangeContentValue(e.target.value)}
                ></Input.TextArea>
              </div>
        ...
        <div className={styles["selectAssistant"]}>
          ...
          <div className={styles["select-assistant"]}>
            Select a chat completion model to start
          </div>
