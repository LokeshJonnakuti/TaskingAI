...
function Retrieval() {
  ...
  const { tooltipEditTitle, tooltipRecordTitle, tooltipMoreTitle } =
    tooltipTitle();
  ...
  const handleEdit = (val: any) => {
    ...
    setSelectedRows(
      val.model_name
        ? [val.model_name + "-" + val.embedding_model_id]
        : ["Untitled Model" + "-" + val.embedding_model_id],
    );
    setSelectedModelName(
      val.model_name ? [val.model_name] : ["Untitled Model"],
    );
  ...
  return (
    <div className={styles["retrieval"]}>
      ...
      />
      <DeleteModal
        describe={`${t("deleteItem")} ${deleteValue || "Untitled Collection"}? ${t("projectRetrievalDeleteDesc")}`}
      ...
      <Drawer
        ...
        open={recordOpen}
        width={1000}
        title={`${drawerName} / ${recordOrChunk === "Chunks" ? t("projectChunks") : t("projectRetrievalColumnRecords")}`}
      >
        {recordOrChunk === "Chunks" ? (
          <ChunkPage collectionId={collectionRecordId} />
        ) : (
          <RecordPage
            fetChData={() => fetchData({ limit })}
            collectionId={collectionRecordId}
          ></RecordPage>
        )}
      </Drawer>
    </div>
