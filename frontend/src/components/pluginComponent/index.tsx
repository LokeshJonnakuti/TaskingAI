...
function PluginComponent(props: any) {
  ...
  const [pluginId, setPluginId] = useState(
    pluginSelectedId !== undefined ? pluginSelectedId : "",
  );
  const [inputValue, setInputValue] = useState("");
  const [bundleItem, setBundleItem] = useState(
    bundleSelectedItem !== undefined ? bundleSelectedItem : {},
  );
  ...
  const handleSelectEndChange = (value: string) => {
    if (value === "All Records") {
      ...
      const data = bundleList
        .map((item: any) => {
          const newDataList = item.plugins.filter((plugin: any) =>
            selectedData.includes(plugin.plugin_id),
          );
          return {
            ...item,
            plugins: newDataList,
        ...
        {bundleListData.length === 0 ? (
          <div className={styles["no-data"]}>
            ...
            <Button
              icon={<PlusOutlined />}
              type="primary"
              className={`${styles["prompt-button"]} next-button`}
              onClick={handleNewBundle}
            >
              New Bundle
            </Button>
          ...
          bundleListData.map((item: any) => {
            return (
              <Collapse
                key={item.bundle_instance_id}
                collapsible="header"
                className={styles.pluginCollapse}
              >
                <Collapse.Panel
                  showArrow={false}
                  header={
                    <div className={styles.bundle}>
                      ...
                      <div className={styles.right}>
                        <div className={styles.name}>
                          <span>{item.name}</span> <RightArrow />
                        </div>
                        <div className={styles.desc}>{item.description}</div>
                        <div className={styles.time}>
                          {formatTime(item.created_timestamp)}
                        </div>
                      </div>
                    </div>
                  }
                ...
                >
                  <div className={styles["plugin-list"]}>
                    {item.plugins.map((plugin: any) => (
                      <div
                        className={`${styles["pluginSingle"]} ${plugin.plugin_id === pluginId && styles["selectItem"]} ${selectedData.includes(plugin.plugin_id) && styles["selectedItem"]}`}
                        onClick={
                          selectedData.includes(plugin.plugin_id)
                            ? undefined
                            : () => handleClickPlugin(plugin, item)
                        }
                      >
                        <div className={styles.pluginName}>
                          ...
                          {selectedData.includes(plugin.plugin_id) ? (
                            ...
                            <Radio checked={plugin.plugin_id === pluginId} />
                          )}
                        </div>
