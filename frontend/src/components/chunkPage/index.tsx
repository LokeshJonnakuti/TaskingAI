...
import { Button, Space, Input, Spin, Tooltip, Modal } from "antd";
  ...
  const columns = [
    {
      ...
      render: (text: string) => (
        <div style={{ display: "flex", alignItems: "center", margin: 0 }}>
          <span style={{ fontSize: "12px", color: "#777" }}>{text}</span>
          <CopyOutlined
            className="icon-copy"
  ...
  return (
    <Spin spinning={loading}>
      ...
      >
        <div className={styles["text-content"]}>
          ...
          <div className={styles["desc"]}>
            {t("projectChunkTextContentDesc")}
          </div>
          <Input.TextArea
            placeholder={t("projectChunkPlaceholder")}
            showCount
            minLength={0}
            maxLength={4096}
            value={contentValue}
      ...
      <DeleteModal
        ...
        describe={`${t("deleteItem")} ${t("projectChunk")} ${deleteId}? ${t("projectDeleteChunkDesc")}`}
        title={t("projectDeleteChunkTitle")}
