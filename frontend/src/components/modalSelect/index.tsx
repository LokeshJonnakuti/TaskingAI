...
function ModalSelect(prop: any) {
  ...
  const {
    ...
    newTitle,
    handleSelectedItem,
    retrievalSelectedList,
    retrievalModal,
    handleClose,
  } = prop;
  const [collectionId, setCollectionId] = useState(
    collectionSelectedId !== undefined ? collectionSelectedId : "",
  );
  ...
  const handleRetrievlCancel = () => {
    if (!collectionSelectedId) {
      ...
      const collectionItems = retrievalList.find(
        (item: any) => item[id] === collectionSelectedId,
      );
      if (collectionItems) {
  ...
  const handleSelectEndChange = (value: string) => {
    if (value === "All Records") {
      ...
      const data = retrievalList.filter((item: any) =>
        retrievalSelectedList.map((item1: any) => item1[id]).includes(item[id]),
      );
      setContentList(data);
    }
  };
          ...
          <Spin spinning={contentLoading}>
            <div className="content">
              {contentList.map((item: any, index: number) => (
                <div
                  className={`retrieval-single ${retrievalSelectedList.map((item1: any) => item1[id]).includes(item[id]) && "selected-retrieval"} ${item[id] === collectionId && "retrieval-selected-single"}`}
                  key={index}
                  onClick={
                    retrievalSelectedList
                ...
                >
                  <div className="top">
                    ...
                    {retrievalSelectedList
                      .map((item1: any) => item1[id])
                      .includes(item[id]) && item[id] !== collectionId ? (
                      <span className="text">Selected</span>
                    ) : (
                      <Radio checked={item[id] === collectionId}></Radio>
                    )}
                  </div>
