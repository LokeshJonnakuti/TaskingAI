...
function ModelsPage() {
  ...
  const { tooltipEditTitle, tooltipPlaygroundTitle, tooltipMoreTitle } =
    tooltipTitle();
  ...
  const content = (
    <div style={{ cursor: "pointer" }}>
      <p className={styles["popover-eidt"]} onClick={handleViewCode}>
        View code
      </p>
      <p
        className={styles["popover-delete"]}
        onClick={() => handleDelete(record as RecordType)}
      >
        Delete
      </p>
    </div>
  ...
  columns.push({
    ...
    render: (_: string, record: RecordType) => (
      <Space size="middle">
        <div
          onClick={
            record.type !== "chat_completion"
              ? undefined
              : () => handleJump(record as RecordType)
          }
          className={`table-edit-icon ${record.type !== "chat_completion" && styles.typeDisabled} `}
        >
          <Tooltip
            placement="bottom"
            title={record.type === "chat_completion" && tooltipPlaygroundTitle}
            color="#fff"
            arrow={false}
  ...
  const handleJump = async (value: RecordType) => {
    ...
    localStorage.setItem(
      "allowedConfigs",
      JSON.stringify(res.data.allowed_configs),
    );
    const res1 = await getModelsForm(value.model_id);
    localStorage.setItem(
      "streaming",
      JSON.stringify(res1.data.properties.streaming),
    );
    dispatch(setLoading(false));
    dispatch(setPlaygroundSelect("chat_completion"));
    navigate(
      `/project/playground?model_id=${value.model_id}&model_name=${value.name}`,
    );
  };
    ...
    await form1.validateFields().then(async () => {
      ...
      await form.validateFields().then(async () => {
        ...
        try {
          ...
          await updateModels(
            modelId,
            modelType === "wildcard" ? wildcardParams : params,
          );
    ...
    <div className={styles["models-page"]}>
      <Spin spinning={loading} wrapperClassName={styles.spinloading}>
        <ModalTable
          ...
          columns={columns}
          ifSelect={false}
          onOpenDrawer={handleCreateModel}
          dataSource={modelList}
        />
      </Spin>
      <ModelModal
        getOptionsList={fetchData1}
        ref={childRef}
        open={modelOne}
      ...
      >
        <Spin spinning={editLoading}>
          <div className={styles["second-modal"]}>
            ...
            >
              {t("projectModelCredentials")}
            </div>
            <div className={styles["label-desc"]}>
              {t("projectModelCredentialsDesc")} {t("referTo")}{" "}
              <a
                className="href"
                href="https://docs.tasking.ai/docs/guide/model/overview#required-credentials-for-model-access"
                target="_blank"
                rel="noopener noreferrer"
              >
                {t("projectModelCredentialsLink")}
              </a>{" "}
              {t("projectModelCredentialsDescEnd")}
            </div>
            {resetButtonShow && (
              <div
              ...
              >
                <div className={styles["text1"]}>
                  {t("projectModelResetCredentials")}
                </div>
              </div>
            )}
            <Form
              ...
              {formData &&
                ...
                Object.entries(formData.properties).map(([key, property]) => (
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: formData.required.includes(key)
                          ? true
                          : false,
      ...
      <ViewCode
        open={viewCodeOpen}
        data={viewCodeData}
        handleClose={handleCloseViewCode}
      />
      <Modal
        title={t("projectDeleteModelTitle")}
      ...
      >
        <p className={styles.desc}>
          {t("deleteItem")}
          <span className={styles.span}> {record.name}</span>?{" "}
          {t("projectDeleteModelDesc")}{" "}
        </p>
        <Input
          value={deleteValue}
          onChange={handleDeleteValue}
          placeholder={t("projectDeleteModelPlaceholder")}
        ></Input>
      </Modal>
    </div>
  );
