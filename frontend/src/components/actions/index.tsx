import { Space, Drawer, Spin, Tooltip } from "antd";
...
function Actions() {
  ...
  const handleEdit = async (val: any) => {
    ...
    if (res.data.authentication) {
      if (res.data.authentication.content) {
        ...
        setAuthentication(
          Object.values(res.data.authentication.content)[0] as string,
        );
      } else {
  ...
  const handleRequest = async () => {
    ...
    try {
      if (actionId) {
        (actionDrawerRef.current as any).getResetButtonState() &&
  ...
  return (
    <div className={styles["actions"]}>
      ...
      <DeleteModal
        ...
        describe={`${t("deleteItem")} ${deleteValue}? ${t("projectActionDeleteDesc")}`}
        title="Delete Action"
