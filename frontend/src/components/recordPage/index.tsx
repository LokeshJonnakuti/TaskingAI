...
function RecordPage({
  ...
  const columns = [
    {
      ...
      render: (text: string, record: any) => (
        <div>
          <p className="table-text" style={{ fontSize: "14px" }}>
            {text || "Untitled"}
          </p>
          <p
            style={{
          ...
          >
            ...
            <CopyOutlined
              className="icon-copy"
              onClick={() => handleCopy(record.record_id)}
            />
          </p>
        </div>
      ),
    },
    {
      ...
      render: (text: string, record: any) =>
        record.type === "text" ? (
          ...
          >
            <WebIcon style={{ marginRight: "4px" }} />
            {JSON.parse(record.content).url}
          </span>
        ) : (
          <span
            style={{
          ...
          >
            {IconReverse(JSON.parse(record.content).file_name)}
            {JSON.parse(record.content).file_name}
          </span>
        ),
    },
    {
      ...
      key: "status",
      width: 180,
      render: (text: string) => <div className={text}>{text}</div>,
    },
    {
      title: `${t("chunk")}`,
    ...
    },
    {
      ...
      render: (_text: string, record: any) => (
        <Space size="middle">
          <div
            onClick={
              record.type !== "file" ? () => handleEdit(record) : undefined
            }
            className={`table-edit-icon ${record.type === "file" && styles.typeDisabled}`}
            style={{
              height: "34px",
              width: "34px",
              padding: "0",
          ...
          >
            <Tooltip
              placement="bottom"
              title={record.type !== "file" && tooltipEditTitle}
              color="#fff"
              arrow={false}
  ...
  const props: UploadProps = {
    ...
    onDrop(e) {
      ...
      const fileExtension = e.dataTransfer.files[0].name.slice(
        ((e.dataTransfer.files[0].name.lastIndexOf(".") - 1) >>> 0) + 2,
      );
  ...
  return (
    <Spin spinning={loading}>
      ...
      <Modal
        ...
        title={drawerTitle}
        centered
        className={styles["record-create-model"]}
        ...
        <div className={styles["text-content"]}>
          ...
          {type === "text" && (
            <>
              <div
                ...
                style={{ display: "flex", alignItems: "center" }}
              >
                <span className={styles["red-span"]}> * </span>{" "}
                {t("projectChunkTextContent")}
              </div>
              <div className={styles["desc"]}>
                {t("projectRecordContentDesc")}
              </div>
              <Input.TextArea
          ...
          {type === "web" && (
            <>
              ...
              <div className={styles["desc"]}>
                The website URL should start with https.
              </div>
              <Input
            ...
            (fileList.length ? (
              <div>
                {fileList.map((item: any, index: number) => (
                  <div key={index}>{customItem(item)}</div>
                ))}
              </div>
            ) : (
              <Dragger {...props}>
                ...
                <p className="ant-upload-text">
                  <span className={styles["click-to-upload"]}>
                    Click to upload
                  </span>{" "}
                  or drag and drop
                </p>
          ...
          <div className={styles["label-desc"]}>
            {t("projectRecordChunkSizeDesc")}
          </div>
          <InputNumber
            className={styles["input-number1"]}
            placeholder={t("projectRecordChunkSizePlaceholder")}
            parser={(value: string | undefined) =>
              isNaN(Number(value)) ? 1 : parseInt(value as string, 10)
            }
            value={chunkSize}
            onChange={(value: number | null) => setChunkSize(value as number)}
            min={100}
            max={500}
          ></InputNumber>
          <div className={styles["label"]}>
          ...
          <div className={styles["label-desc"]}>
            {t("projectRecordChunkOverlapDesc")}
          </div>
          <InputNumber
            className={styles["input-number"]}
            placeholder={t("projectRecordChunkOverlapPlaceholder")}
            value={chunkOverlap}
      ...
      <DeleteModal
        ...
        describe={`${t("deleteItem")} ${t("projectRecord")} ${deleteId}? ${t("projectDeleteChunkDesc")}`}
        title={t("projectRecordDeleteRecord")}
