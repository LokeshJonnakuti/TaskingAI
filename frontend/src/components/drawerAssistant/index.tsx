...
const DrawerAssistant = forwardRef((props: any, ref: any) => {
  ...
  const {
    ...
    openDrawer,
    selectedPluginGroup,
    selectedActionsSelected,
    handleNewActionModal,
    topk,
    maxTokens,
    bundilesList,
    handleMaxToken,
  ...
  const [selectedPluginList, setSelectedPluginList] =
    useState(selectedPluginGroup);
  const [retrievalSelectedList, setRetrievalSelectedList] = useState<any[]>(
    selectedCollectionList,
  );
  const [actionSelectedList, setActionSelectedList] = useState<any[]>(
    selectedActionsSelected,
  );
  const [pluginActionList, setPluginActionList] =
    useState<any[]>(selectedActionsRows);
  const [isShowRetrievalConfig, setIsShowRetrievalConfig] = useState(false);
  const [retrievalFormList, setRetrievalFormList] = useState<any[]>(
    selectedCollectionList.map((item: any) => ({
  ...
  useEffect(() => {
    const data = retrievalFormList.filter((item: any) =>
  ...
  useEffect(() => {
    setPluginActionList(
      selectedActionsRows.length
        ? selectedActionsRows
        : [{ type: "plugin", value: "" }],
    );
  }, [selectedActionsRows]);
  useImperativeHandle(ref, () => ({
    getRetrievalSelectedList: () =>
      retrievalSelectedList.map((item) => item?.collection_id),
  ...
  const handleSelectActions = (index: any, actionId: string) => {
    if (actionId) {
      ...
      setActionSelectedList((prev) => {
        const newRetrievalSelectedList = prev.filter(
  ...
  const handleSelectPlugins = (index: any, value: string) => {
    if (value) {
      ...
      setSelectedPluginList((prev: any) => {
        const newActionFormList = prev.filter(
          (item: any) => item !== value.split("/")[1],
        );
  ...
  const handleDeleteRetrieval = (id: string, index: number) => {
    setRetrievalFormList((prev) => {
      if (!id) {
        ...
        const newRetrievalFormList = prev.filter(
  ...
  const handleDeleteTool = (data: any, index: number) => {
    setPluginActionList((prev) => {
      if (!data.value) {
        ...
        const newPluginActionList = prev.filter(
    ...
    if (data.type === "action") {
      setActionSelectedList((prev) => {
        const newActionSelectedList = prev.filter(
          (item) => item.action_id !== data.value,
        );
        return newActionSelectedList;
      });
    }
    if (data.type === "plugin") {
      setSelectedPluginList((prev: any) => {
        const newPluginSelectedList = prev.filter(
          (item: any) => item !== data.value.split("/")[1],
        );
  ...
  const handlePluginConfirm = (pluginId: any, data: any) => {
    const pluginName = data.plugins.find(
      (item: any) => item.plugin_id === pluginId,
    ).name;
    setSelectedPluginList((prev: any) => {
  ...
  return (
    <div className="drawer-assistant">
      <div className="left-assistant">
        ...
        <div className="name-prompt">{t("projectModelColumnName")}</div>
        <Input
          value={drawerName}
        ...
        ></Input>
        <div className="label">{t("projectAssistantsColumnDescription")}</div>
        <div className="label-desc">
          The description should serve as an internal note to clarify the
          purpose of the assistant; it will not be used as a system prompt.
        </div>
        <Input.TextArea
          ...
          maxLength={200}
          value={drawerDesc}
          onChange={(e) => handleDescriptionChanges(e as any)}
        />
        ...
        <div className="label-desc">
          {t("projectAssistantsLanguageModelDesc")}
        </div>
        <Select
          ...
          className="input"
          value={modelName}
          onClick={handleSelectModelIds}
        ></Select>
        <div className="label">
          <span>{t("projectSystemPromptTemplate")}</span>
        </div>
        <div className="label-desc">
          A system prompt is the initial instruction that guides assistant’s
          response. Prompt variables can be defined using double curly brackets,
          like {"{{language}}"}. Those variables are then populated during the
          generation process. &nbsp;
          <a
            className="referToTheDocumentationFor href"
            href="https://docs.tasking.ai/docs/guide/product_modules/assistant/components/system-prompt-template"
            target="_blank"
          >
        ...
        {systemPromptTemplate?.map((value: any, index: number) => (
          <div className="input-map" key={index}>
            ...
            />
            <DeleteInputIcon
              onClick={() => handleDeletePromptInputs(index)}
              style={{ marginTop: "8px" }}
            />
          </div>
        ))}
        <div className="add-bottom">
          <Button
            ...
            disabled={systemPromptTemplate.length === 10}
            icon={<PlusOutlined />}
          >
            Add
          </Button>
        ...
        <div className="label-desc">
          The context memory allows the assistant to remember past
          conversations, thereby enabling stateful invocations. &nbsp;
          <a
            className="referToTheDocumentationFor href"
            href="https://docs.tasking.ai/docs/guide/product_modules/assistant/components/memory"
            target="_blank"
          >
            <span className="referToThe">Learn more</span>
          </a>
          ...
          <div className="input-double">
            ...
            <div className="input-children1">
              ...
              <InputNumber
                ...
                min={1}
                max={8192}
                placeholder="From 1 to 8192"
                value={inputValue2}
                onChange={(e) => handleInputValue2(e as number)}
              ></InputNumber>
            </div>
          </div>
        )}
      ...
      <div className="right-assistant">
        <div className="basic-information">Integrations</div>
        <div className="desc-retrieval">{t("projectRetrievalTitle")}</div>
        <div className="label-desc">
          Incorporate retrieval sources into your assistant to facilitate
          effective retrieval-augmented generation.{" "}
          <a
            href="https://docs.tasking.ai/docs/guide/product_modules/retrieval/overview"
            target="_blank"
            className="referToTheDocumentationFor"
          >
            Learn more
          </a>
        </div>
        {retrievalFormList.map((item: any, index: number) => (
          <div className="retrieval-list" key={item.collection_id}>
            <Select
              ...
              className="retrieval-type"
            ></Select>
            <Select
              className="input"
              mode="multiple"
              placeholder={
                !item.name && t("projectAssistantRetrievalPlaceHolder")
              }
              style={{ caretColor: "transparent" }}
              onClick={() => handleCollectionModal(index, item.collection_id)}
              suffixIcon={<RightOutlined />}
              open={false}
            ...
            <div>
              {" "}
              <DeleteInputIcon
                onClick={() => handleDeleteRetrieval(item.collection_id, index)}
                style={{ marginTop: "8px" }}
              />
            </div>
          </div>
        ...
        <div
          className={`add-bottom ${retrievalFormList.length === 10 && "disabled-button"}`}
        >
          <Button
            onClick={handleAddRetrieval}
            disabled={retrievalFormList.length === 10}
            icon={<PlusOutlined />}
          >
            Add
          </Button>
            ...
            <div className="input-double">
              ...
              <div className="input-children1">
                ...
                <InputNumber
                  ...
                  placeholder="From 1 to 8192"
                  value={maxTokens}
                  onChange={(e) => handleMaxTokens(e as number)}
                ></InputNumber>
              </div>
            </div>
          </>
        ...
        <div className="hr"></div>
        <div className="desc-retrieval">Tools</div>
        <div className="label-desc">
          Integrate tools into your assistant to extend its capabilities to
          address different user needs.{" "}
          <a
            href="https://docs.tasking.ai/docs/guide/product_modules/tool/overview"
            target="_blank"
            className="referToTheDocumentationFor"
          >
            {" "}
          ...
          return (
            <div className="retrieval-list">
              <Select
                ...
                defaultValue="plugin"
                className="retrieval-type"
                onChange={(value) => handleToolsChange(value, index, item)}
                value={item.type}
              ></Select>
              <Select
                placeholder={
                  item.type === "plugin" ? "Select a plugin" : "Select a action"
                }
                open={false}
                removeIcon={null}
                mode="multiple"
                ...
                onClick={
                  item.type === "action"
                    ? () => handleSelectActions(index, item.value)
                    : () => handleSelectPlugins(index, item.value)
                }
              ></Select>
              <div>
                {" "}
                <DeleteInputIcon
                  onClick={() => handleDeleteTool(item, index)}
                  style={{ marginTop: "8px" }}
                />
              </div>
            </div>
        ...
        <div
          ...
          className={`add-bottom ${pluginActionList.length === 10 && "disabled-button"}`}
        >
          <Button
            onClick={handleAddTool}
            disabled={pluginActionList.length === 10}
            icon={<PlusOutlined />}
          >
            Add
          </Button>
      ...
      {collectionModal && (
        <ModalSelect
          ...
          id="collection_id"
          handleNewModal={handleNewCollection1}
          title="Collection"
          nameTitle="Select Collection"
          newTitle="New collection"
          hasMore={collectionHasMore}
          handleSelectedItem={handleSelectedItem}
          retrievalModal={collectionModal}
          retrievalSelectedList={retrievalSelectedList}
          retrievalList={retrievalList}
          handleClose={handleCancelCollectionModal}
        ></ModalSelect>
      )}
      {actionModal && (
        <ModalSelect
          id="action_id"
          collectionSelectedId={actionSelectedId}
          nameTitle="Select Action"
          handleNewModal={handleNewAction}
          title="Action"
          newTitle="New action"
          hasMore={actionHasMore}
          handleSelectedItem={handleActionItem}
          retrievalModal={actionModal}
          retrievalSelectedList={actionSelectedList}
          retrievalList={actionList}
          handleClose={handleCancelActionModal}
        ></ModalSelect>
      )}
      {pluginModal && (
        <PluginComponent
          bundleSelectedItem={bundleSelectedItem}
          pluginSelectedId={pluginSelectedId}
          selectedData={selectedPluginList}
          handleSelectedItem={handlePluginConfirm}
          open={pluginModal}
          handleCreateBundle={handleCreateBundle}
          bundleList={bundilesList}
