...
function ModalTable({
  ...
  const [selectedRowKeys, setSelectedRowKeys] = useState(
    defaultSelectedRowKeys && defaultSelectedRowKeys.length
      ? defaultSelectedRowKeys
      : [],
  );
  useEffect(() => {
    const updateScroll = () => {
      ...
      let tableContainerH: HTMLElement | null = document.querySelector(
        ".ant-table-container",
      );
      let modalInnerTable: HTMLElement | null =
        document.querySelector(".modal-inner-table");
      let modalOpen = true;
      if (modalInnerTable) {
        modalOpen = getComputedStyle(modalInnerTable).display === "none";
      }
      let elementsWithPrefix: any = document.querySelectorAll(
        '[class*="_drawer-inner-table"]',
      );
      let elementsWithPrefix1: any = document.querySelectorAll(
        '[class*="_drawer-inner-chunk"]',
      );
      let containerWidth = 0;
      let containerHeight = 0;
      if (
        !modalInnerTable &&
        elementsWithPrefix.length === 0 &&
        elementsWithPrefix1.length === 0
      ) {
        setTimeout(() => {
          if (!tableContainer) {
            return;
        ...
        setTimeout(() => {
          ...
          let antDrawerBodyClassName =
            firstElement.querySelector(".ant-drawer-body");
          let tableClassName = firstElement.querySelector(
            ".ant-drawer-body table",
          );
        ...
        setTimeout(() => {
          const tableContainer: HTMLElement | null = document.querySelector(
            ".modal-inner-table .ant-table-container table",
          );
          const tableContainerH: HTMLElement | null = document.querySelector(
            ".modal-inner-table .ant-table-container",
          );
  ...
  useEffect(() => {
    if (defaultSelectedRowKeys && defaultSelectedRowKeys.length) {
      const tag = defaultSelectedRowKeys.map((item) =>
        item.split("-").pop(),
      ) as any[];
  ...
  const onSelectChange1 = (record: any) => {
    ...
    if (mode === "multiple") {
      ...
      newSelectedRowKeys = isSelected ? [] : [key];
    }
  ...
  const handleRowClick = (record: any) => {
    ...
    if (mode === "multiple") {
      newSelectedRowKeys = isSelected
        ? selectedRowKeys.filter((selectedKey) => selectedKey !== key)
        : [...selectedRowKeys, key];
      const data = dataSource?.filter((row) =>
    ...
    <div className={styles["modal-table"]} key={name}>
      {!ifHideLeftHeader ? (
        <div className={styles["header-table"]}>
          <Select
            defaultValue={"ID"}
            onChange={handleSelectFrontChange}
            options={ifOnlyId ? optionsFront1 : optionsFront}
          ...
          <Input
            ...
            value={inputValue}
          />
          <Button className="cancel-button" onClick={handleSearch}>
            Search
          </Button>
          {ifSelect && mode === "multiple" && (
            <Select
      ...
      <div className={styles["table-border"]}>
        ...
        {!ifHideFooter && (
          <div
            className={`${styles.footer} ${ifSelect ? styles["footer-position"] : ""}`}
          >
            <Button
              className={`${styles["previous-button"]} ${!previousButtonDisabled && styles["able-click-button"]}`}
            ...
            <Button
              className={`${styles["next-button-group"]} ${!nextButtonDisabled && styles["able-click-button"]}`}
              onClick={handleNext}
