...
const ViewCode = (props: any) => {
  ...
  useEffect(() => {
    if (flattenedData.length > 0) {
      const name = Array.from(
        new Set(flattenedData.map((item: any) => item.language_name)),
      )[0];
      ...
      const newSetList =
        Array.from(
          new Set(
            flattenedData
              .filter((item: any) => item.language_name === name)
              .map((item: any) => item.part_name),
          ),
        ) || [];
      setFilteredParts(
        Array.from(
          new Set(
            flattenedData.filter((item: any) => item.language_name === name),
          ),
        ) || [],
      );
      const templateName: any = Array.from(
        new Set(
          flattenedData.filter((item: any) => item.language_name === name),
        ),
      )[0];
      if (templateName) {
        ...
        setRightContent([
          flattenedData.filter(
            (item: any) => item.template_name === templateName.template_name,
          )[0],
        ]);
      }
  ...
  const handleLanguageSelect = (language: any) => {
    ...
    const newSetList =
      Array.from(
        new Set(
          flattenedData
            .filter((item: any) => item.language_name === language)
            .map((item: any) => item.part_name),
        ),
      ) || [];
    setUniqueParts(newSetList);
    ...
    setFilteredParts(
      Array.from(
        new Set(
          flattenedData.filter((item: any) => item.language_name === language),
        ),
      ) || [],
    );
    const templateName: any = Array.from(
    ...
    if (templateName) {
      ...
      setRightContent([
        flattenedData.filter(
          (item: any) =>
            item.template_name === templateName.template_name &&
  ...
  const handlePartSelect = (part: any) => {
    ...
    setRightContent(
      filteredParts.filter(
        (item: any) => item.template_name === part.template_name,
      ),
    );
  };
  useEffect(() => {
      ...
      <ul style={{ paddingLeft: 0 }}>
        {items.map((item: any) => (
          <li key={item.id} style={{ display: "flex" }}>
            <div>
              {item.title} {item.level === 2 && <ArrowTop />}
            </div>
  ...
  return (
    <Modal
      ...
      open={open}
      title="View Code"
      className={`${styles.modal} view-code-component`}
      ...
      >
        <div className={styles.silder}>
          {uniqueLanguages.map((language: any) => (
            <div
              className={`${styles["first-menu"]} ${language === languageName && styles["selected-menu"]}`}
              key={language}
        ...
        <div className={styles.content}>
          {selectedLanguage &&
            uniqueParts.map((part: any, index: number) => (
              <div key={part} style={{ width: "180px" }}>
                <div
                  className={styles["second-menu"]}
                ...
                {isShowSubMenu[index] && (
                  <ul>
                    {filteredParts
                      .filter((item: any) => item.part_name === part)
                      .map((template: any) => (
                        <li
                          className={`${styles["third-menu"]} ${templateName === template.template_name && styles["selected-menu"]}`}
                          key={template.template_name}
                          onClick={() => handlePartSelect(template)}
                        >
                ...
                >
                  {item.variables.map((variable: any, index: number) => (
                    <div className={styles.variables} key={index}>
                      <div className={styles["variables-left"]}>
                        {variable.replace(
                          /\$\$(.*?)\$\$/g,
                          (_match: any, p1: any) => p1.toLowerCase(),
                        )}
                      </div>
                      <Input
                        type="text"
                        onChange={(e) => handleValue(e, item.id, variable)}
                        className={styles["variable-input"]}
                        placeholder={`Enter ${variable.replace(/\$\$(.*?)\$\$/g, (_match: any, p1: any) => p1.toLowerCase())}`}
                      />
                    </div>
                  ))}
                </div>
