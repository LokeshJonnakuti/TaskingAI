...
const getAiModelsList = async (
  ...
  if (type) {
    return await request.get(
      `${project_base_url}/model_schemas?offset=${offset}&limit=${limit}&type=${type}&provider_id=${providerId}`,
    );
  } else {
    return await request.get(
      `${project_base_url}/model_schemas?offset=${offset}&limit=${limit}&provider_id=${providerId}`,
    );
  }
};
...
const getAiModelsForm = async (id: string) => {
  const project_base_url = `api/v1`;
  return await request.get(
    `${project_base_url}/providers/get?provider_id=${id}`,
  );
};
const getModelSchema = async (modelSchemaId: string) => {
  const url = `api/v1`;
  return await request.get(
    `${url}/model_schemas/get?model_schema_id=${modelSchemaId}`,
  );
};
const getModelsList = async <T extends Record<string, string | number>>(
  ...
  if (type) {
    return await request.get(
      `${project_base_url}/models?${str}&equal_filter={"type": "${type}"}`,
    );
  } else {
...
const getModelsForm = async (id: string) => {
  const project_base_url = `api/v1`;
  return await request.get(
    `${project_base_url}/models/${id}?include_credentials_schema=true&include_display_credentials=true`,
  );
};
const getModelProviderList = async (type: string) => {
  ...
  if (type) {
    return await request.get(
      `${project_base_url}/providers?type=${type}&limit=100`,
    );
  } else {
