...
class TestBundleInstance(Tool):
        ...
        create_bundle_instance_dict_list = [
            {
                ...
                "credentials": {"EXCHANGERATE_API_API_KEY": CONFIG.EXCHANGERATE_API_API_KEY},
                "bundle_id": "exchangerate_api",
            },
            {
                "name": "open_weather",
                "credentials": {"OPEN_WEATHER_API_KEY": CONFIG.OPEN_WEATHER_API_KEY},
    ...
    async def test_list_bundle_instances(self):
        ...
        list_bundle_instances_data_list = [
            {"limit": 10, "order": "desc", "after": Tool.bundle_instance_id},
            {"limit": 10, "order": "asc", "prefix_filter": json.dumps({"name": Tool.bundle_instance_name[:4]})},
            {
                "limit": 10,
                "order": "desc",
    ...
    async def test_success_delete_bundle_instance(self):
        ...
        for bundle_instance in bundle_instances_data:
            ...
            assume_error(
                get_res, {"except_http_code": "404", "except_status": "error", "except_error_code": "OBJECT_NOT_FOUND"}
            )
