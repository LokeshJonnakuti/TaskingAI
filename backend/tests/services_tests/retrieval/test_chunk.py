import pytest
import json
from backend.tests.api_services.retrieval.chunk import (
    query_chunks,
...
class TestChunk(Retrieval):
    ...
    async def test_query_chunk(self):
        query_chunks_data_list = [
            {"query_text": "TaskingAI", "top_k": 1, "score_threshold": 0.01},
            {"query_text": "TaskingAI", "top_k": 1, "max_tokens": 20000},
        ]
    ...
    async def test_create_chunk(self):
        create_chunk_data_list = [
            {"content": "This is a test for create chunk.", "metadata": {"key1": "value1"}},
            {
    ...
    async def test_list_collection_chunks(self):
        list_collection_chunks_data_list = [
            {"limit": 10, "order": "desc", "after": Retrieval.chunk_id},
            {
                "limit": 10,
                "order": "asc",
    ...
    async def test_list_record_chunks(self):
        if "TASKINGAI_WEB" in CONFIG.TEST_MODE:
            list_record_chunks_data = {"limit": 10, "order": "desc", "id_search": Retrieval.chunk_id[:4]}
