...
from backend.tests.api_services.assistant.message import (
    create_message,
    get_message,
    list_messages,
    update_message,
...
class TestMessage(Assistant):
    ...
    async def test_create_message(self):
        create_message_data_list = [
            {"role": "user", "content": {"text": "hello, can you help me?"}},
            {"role": "user", "content": {"text": "what is the meaning of 123?"}, "metadata": {"x": "x"}},
        ]
    ...
    async def test_list_messages(self):
        list_messages_data = {"limit": 10, "order": "asc", "before": Assistant.message_id}
    ...
    async def test_update_message(self):
        update_message_data = {"metadata": {"xxx": "xxx"}}
